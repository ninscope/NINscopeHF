...
uint16_t		CyPyAddress,CyPyData;
CyBool_t 		fCyPythonUpd;
		...
		)
{
	...
	if(fCyPythonUpd)
	{
		CyFxSpiPythonWord(CyPyAddress,CyPyData);
		fCyPythonUpd = CyFalse;
	}
}
...
CyFxUVCApplnSPIInit (void)
{
	...
	spiConfig.clock      = 30000;		// 30kHz The Serializer can handle up to 66kHz through the GPIO
...
//						}
						...
						switch( inBuf_p.buffer[1] )
						{
							...
							case CMD_PYTHON:		CyPyAddress = CY_U3P_MAKEWORD(inBuf_p.buffer[2],inBuf_p.buffer[3]);
													CyPyData =    CY_U3P_MAKEWORD(inBuf_p.buffer[4],inBuf_p.buffer[5]);
													fCyPythonUpd = CyTrue;
													break;

							case CMD_RESET_FRAME:	//Back to default settings
							    					CyFxSpiPythonWord(195,1);
							    					CyFxSpiPythonWord(192,0x0801);
							    					CyPythonROISet();
							    					CyPythonFrameSet();
							    					CyPythonDefaults();
							    					CyU3PEventSet(&glFxUVCEvent, CY_FX_UVC_DMA_RESET_EVENT_NOT_ACTIVE, CYU3P_EVENT_OR);
													break;
	            ...
	            if(eventFlag & CY_FX_UART_TEMP_DATA_EVENT)
		            ...
		            if(eventFlag & CY_FX_UART_VERSION_EVENT)
					{
									...
									outBuf_p.buffer[outBuf_p.count++] = '.';
									outBuf_p.buffer[outBuf_p.count++] = '1';
									outBuf_p.buffer[outBuf_p.count++] = 'H';
									outBuf_p.buffer[outBuf_p.count++] = 'F';
