...
CyPythonSpiUploads( void )
{
	  ...
	  CyFxSpiPythonWord(65, 0x382B);    //Bias Configuration DEF 0xF8CB IDS 0x382B SCR 0x382B
	  ...
	  CyFxSpiPythonWord(66, 0x53C8);	//AFE Bias Configuration DEF 0x53C8 IDS 0x53C8  SCR 0x53C8
	  ...
	  CyFxSpiPythonWord(67, 0x0665);	//Column Multiplexer Bias Configuration DEF 0x8788 IDS 0x0565 SCR 0x0665
	  ...
	  CyFxSpiPythonWord(68, 0x0085);    //LVDS Bias Configuration DEF 0x0085 IDS 0x0085 SCR 0x0085
										//[3:0] lvds_ibias 0x5 5 LVDS Ibias //P1300
										//[7:4] lvds_iref  0x8 8 LVDS Iref  //P1300

	  CyFxSpiPythonWord(69, 0x0888);    //LVDS Bias Configuration DEF 0x0088 IDS 0x0088 SCR 0x0888
	  ...
	  //CHARGE PUMP
	  CyFxSpiPythonWord(72, 0x4126);	//Charge Pump Configuration DEF 0x2220 IDS 0x127 SCR 0x0117
	  ...
	  CyFxSpiPythonWord(204, 0x00E4);  //Gain Configuration DEF 0x01E1 IDS 0x00E3 SCR 0x00E1
...
CyPythonSoftPowerUp ( void )
{
	  ...
	  CyFxSpiPythonWord(32, 0x7007);  // Divide by 5 off// CyFxSpiPythonWord(32, 0x7007);  // Enable analog clock
...
{
	CyFxSpiPythonWord(199, 72); // Mult- Timer
	CyFxSpiPythonWord(200, 30500); // fr lenght 30fps
}

void
CyPythonROISet ( void )
{
		CyFxSpiPythonWord(256,0xC207);  //7-194  (194-7+1=188*2*2=752)
		CyFxSpiPythonWord(257,0x8810);	//16-136 (136-16=120*2*2=480)

		CyFxSpiPythonWord(258,0xB415);  //21-180 (180-21+1=160*2*2=640)
		CyFxSpiPythonWord(259,0x8710);  //16-136 (136-16=120*2*2=480)

		CyFxSpiPythonWord(260,0x8C3D);  //61-140 (140-61+1=80*2*2=320)
		CyFxSpiPythonWord(261,0x6A2E);  //46-106  (106-46=60*2*2=240)

		//CyFxSpiPythonWord(262,0xC801);  //0-201 (200-1+1*2*2=800)
		//CyFxSpiPythonWord(263,0x4F49);  //73-79   (79-73)=6*2*2=24)

		CyFxSpiPythonWord(262,0x9633);  // (151-50+1=100*2*2=400)
		CyFxSpiPythonWord(263,0x5246);  //   (82-70)=12*2*2=48)

		CyFxSpiPythonWord(264,0xC414);  //lsb from ROI0-Roi1
		CyFxSpiPythonWord(265,0x1414);  //lsb from ROI2-Roi3
}

void
CyPythonDefaults ( void )
{
	CyFxSpiPythonWord( 205, 384 );		// Digital Gain
	CyFxSpiPythonWord( 201, 30400);		// Exposure
	CyFxSpiPythonWord( 195 ,0x0001);		//ROI 1
        ...
        void)
{
	...
	//init LSM6DSLTR
	buf[0] = 0b01000000;						//ODR Output Data Rate 104 Hz
